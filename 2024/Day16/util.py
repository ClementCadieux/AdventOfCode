...
def scoreTile(scores, maze, i, j, score, direction, seen):
    if scores[-2][1] != 0 and score > scores[-2][1]:
        return False
    
    if (i, j) in seen or scores[i][j] != 0 and scores[i][j] < score:
        return False
    ...
    if maze[i][j - 1] != "#" and rightScore != -1:
        right = scoreTile(scores, maze, i, j - 1, rightScore, 1, seen)
    if maze[i + 1][j] != "#" and upScore != -1:
        up = scoreTile(scores, maze, i + 1, j, upScore, 0, seen)
    if maze[i][j + 1] != "#" and leftScore != -1:
        left = scoreTile(scores, maze, i, j + 1, leftScore, 3, seen)
    if maze[i - 1][j] != "#" and downScore != -1:
        down = scoreTile(scores, maze, i - 1, j, downScore, 2, seen)
